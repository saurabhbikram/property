...
#system("aws s3 cp s3://ukpd/property-analysis/alllistings.rds . --profile sb")
```
...
for(x in rsales$property_key) {
  ...
  aa <- filter(all_lisings, property_key==x & 
    ...
    bind_rows(a) %>% arrange(desc(trans_date))
...
```
```{r}
summary(about_sale$time_to_sell)
```
...
```{r}
all_lisings %>% filter(property_key==27201023) %>% 
  select(listing_status, listing_key, listing_date, date_scraped, trans_date, price_num) %>% arrange(desc(trans_date))
```
...
```{r}
mean(about_sale$avg_price_reduction<0.99,na.rm = T)
```
...
plotdf <- about_sale %>% 
         filter(
           ...
           time_to_sell > 10,
           #!(avg_price_reduction>0.95 & time_to_sell>365)
           ) %>% 
  mutate(
    beds=as.factor(ifelse(beds >= 3, "3+", as.character(beds)))
  )
