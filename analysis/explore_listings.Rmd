...
```{r}
table(listings$status %>% tolower) %>% sort
```
...
listings %>% 
  mutate(info_type=case_when(
    ...
    TRUE ~ info_type)) %>% 
  pull(info_type) %>% tolower %>% table %>% sort
```
...
prices_data_sum <- listings %>% 
  ...
  mutate(
    ...
    # create factors, keeping the more importat levels first, since we will use this as the order to 
    # de-duplicate
    ...
    status_clean = case_when(
      ...
      str_detect(tolower(status),"listedforsale") ~ "For Sale", # this is the 'archived' info on website, displaying when something was listed for sale historically
...
prices_data_sum <- prices_data_sum %>% 
...
```{r}
prices_data_sum %>% filter(property_id==29600092) %>% select(status, listing_id, property_id, info_type, status_clean, price_num, saledate_date, area, beds) %>% distinct %>% kable
```
...
```{r}
t <- prices_data_sum %>% filter(property_id %in% 28362000) %>% 
  arrange(year(saledate_date),month(saledate_date),status_clean)
# only show distinct data based on property id, year-month of listing.
distinct(t, property_id, status_clean, year(saledate_date), .keep_all = T) %>% 
  select(listing_id, property_id, info_type, status_clean, price_num, saledate_date) %>% kable
```
...
for(i in seq_along(sale_prices$saledate)) {
...
rental_yields <- sale_prices %>% left_join(rent_prices, by=c("saledate_rent"="saledate", "property_id")) %>% 
  mutate(date_diffs = abs(saledate-saledate_rent)) %>% filter(date_diffs<365*3) %>% 
  mutate(
...
p <- ggplot(prdf, aes(rental_yield, colour=beds, fill=beds)) + geom_density(alpha=0.1, linetype="solid", size=0.1, adjust=1.5) + facet_wrap("year_d", ncol=1,scales = "free_y") +
  ...
  geom_text(data=prdf_sum %>% filter(indicator=="avg"), aes(colour=beds, label=label, y=0), size=1.5, angle=90, vjust=0, hjust=0) +
  ...
  theme_minimal() + theme(panel.grid = element_blank(), axis.text = element_blank(), strip.text = element_text(hjust=0,size = 8), 
                          legend.position = c(0.9,1), legend.text = element_text(size=6), legend.key.size = unit("0.3","cm"), legend.background = element_rect(colour="grey80", size=0.2, linetype="dotted"), legend.title = element_blank()) + scale_x_log10("") + ggtitle(glue("Rental yields each year by # of beds"),subtitle = "distributions, with lines showing average per bed") + scale_y_continuous("")
